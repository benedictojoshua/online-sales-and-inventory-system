...
{
	$query=mysql_query("insert into users(name,email,contactno,password, status) values('$name','$email','$contactno','$password','Pending')");
	if($query)
	{
		$insertd=mysql_query("SELECT id FROM users WHERE email='$email'");
		
		while($resultw=mysql_fetch_array($insertd))
		{
			$idw=$resultw['id'];
		}
		...
		<a href="http://shoppingportal256.000webhostapp.com/update_reg.php?pid=' . $idw . '">Clink this link to verify your email</a>
...
if(isset($_POST['login']))
{
   $email=$_POST['email'];
   $password=md5($_POST['password']);
$query=mysql_query("SELECT * FROM users WHERE email='$email' and password='$password' and status!='Pending'");
...
<html lang="en">
	<head>
	    ...
	    <title>Fashion HAUL Boutique | Signi-in | Signup</title>
...
<div class="form-group">
	    	...
	    	<input type="text" class="form-control unicase-form-control text-input" id="contactno" name="contactno" maxlength="11" onkeypress="return isNumberKey(event)" required >
	  	</div>
